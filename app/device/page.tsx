...
import type { Anchor, ClassifyResponse, DailySchedule, ResponseType } from '@/types'
import { FALLBACK_PHRASES } from '@/types'

const PATIENT_NAME = 'Robert'
...
export default function DevicePage() {
  ...
  const { anchors, fuzzyMatch, refreshCache } = useAnchorCache(DEMO_PATIENT_ID)
  const { updatedAnchors, isConnected } = useRealtimeAlerts(DEMO_PATIENT_ID)
  ...
  useEffect(() => {
    ...
    async function loadSchedule() {
      try {
        ...
        // Schedule unavailable — reminders will be empty
      }
    }
        ...
        try {
          await fetch('/api/alert', {
            ...
            body: JSON.stringify({
              ...
              detail: `${PATIENT_NAME} asked about ${matchedAnchor.label}: "${transcription}"`,
              is_critical: true,
            }),
          })
        } catch {
          // Alert delivery failed — interaction still logged
        }
      }
    },
    [enqueue]
  ...
  const deliverResponse = useCallback(
    async (text: string, photoUrl: string | null) => {
      ...
      try {
        ...
        // TTS failed — text is still visible on screen
      }
    },
    []
  )
    ...
    async (transcript: string) => {
      ...
      try {
        ...
        if (matchedAnchor) {
          ...
          const fallbackText = FALLBACK_PHRASES[key]

          await deliverResponse(fallbackText, null)
          await logInteraction(transcript, null, confidence, key, fallbackText, false)
        }
      } catch {
        await deliverResponse(FALLBACK_PHRASES.fallback_b, null)
      } finally {
        setIsProcessing(false)
      }
    },
    [isProcessing, isOnline, anchors, fuzzyMatch, schedule, deliverResponse, logInteraction]
  )
  ...
  const handleCaregiverName = useCallback(async () => {
    ...
    try {
      const text = FALLBACK_PHRASES.caregiver_name
      await deliverResponse(text, null)
      await logInteraction('Margaret', null, null, 'caregiver_name', text, false)
    } catch {
      // Caregiver name response failed silently
    } finally {
      setIsProcessing(false)
    }
  }, [isProcessing, deliverResponse, logInteraction])

  const { isListening } = useVoiceListener({
    onTranscript: handleTranscript,
    onCaregiverName: handleCaregiverName,
    enabled: started && !muted,
  })
  ...
  return (
    <div className="fixed inset-0 bg-mirsa-device select-none overflow-hidden">
      <AmbientDisplay patientName={PATIENT_NAME} reminders={reminders} />
